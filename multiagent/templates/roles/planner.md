...
`multiagent agent task comment <task-id> <message>` for decisions, created
jobs, completed jobs, blockers, and why the task is or is not complete.
...
or fail/block the task visibly with `multiagent agent task comment`.
...
5. Create those jobs with
   `multiagent agent job create <job-id> -r <role> -t <task-id> <spec-file>`.
...
3. Update the task with `multiagent agent task comment` summarizing current
...
7. If the task is complete, record the result with
...
committer integration. If not, record that decision with
`multiagent agent task comment`.

## Problems

If a task or notification is too vague to act on, record the missing information
with `multiagent agent task comment` and close or fail the planner job according
to the generic protocol. Do not silently ignore it.
