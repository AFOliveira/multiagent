...
def registry_dir() -> Path:
    return Path(os.environ.get("MULTIAGENT_REGISTRY_DIR", DEFAULT_REGISTRY_DIR)).expanduser()
...
def registry_state_dir(repo_root: Path) -> Path:
    state_override = os.environ.get("MULTIAGENT_STATE_DIR", "").strip()
    if state_override:
...
def local_manages_registry() -> bool:
    return os.environ.get("MULTIAGENT_CONTAINER", "").strip() != "docker"
...
def supervisor_pid_is_running(pid: int | None) -> bool:
    ...
    if not cmdline:
        return True
    if "multiagent.cli" in cmdline and "_supervisor" in cmdline:
        return True
...
def run_runtime_tool(repo: Repo, tool: str, args: list[str]) -> int:
    ...
    env = os.environ.copy()
    env["MULTIAGENT_ROOT"] = str(repo.config_dir)
    env["MULTIAGENT_REPO_ROOT"] = str(repo.root)
    env["MULTIAGENT_STATE_DIR"] = str(repo.state_dir)
...
def trust_recorded_pids(repo: Repo) -> bool:
    ...
    if not container_runtime:
        return True
    return os.environ.get("MULTIAGENT_CONTAINER") == container_runtime
...
def cmd_supervisor(args: argparse.Namespace) -> int:
    ...
    multiagent_dir = repo.config_dir
    for name in STATE_SUBDIRS:
        ...
        json.dumps(
            {
                ...
                "container_runtime": os.environ.get("MULTIAGENT_CONTAINER", ""),
                "container_name": os.environ.get("MULTIAGENT_CONTAINER_NAME", ""),
            },
            indent=2,
        )
        + "\n",
    ...
    def launch(label: str, command: list[str]) -> subprocess.Popen[bytes]:
        ...
        env = os.environ.copy()
        env["MULTIAGENT_REPO_ROOT"] = str(root)
        env["MULTIAGENT_ROOT"] = str(multiagent_dir)
        env["MULTIAGENT_STATE_DIR"] = str(state_dir)
        env["MULTIAGENT_INSTANCE_ID"] = read_text(state_dir / "instance-id", state_dir.name, 4096).strip() or state_dir.name
        try:
            proc = subprocess.Popen(
                command,
                cwd=multiagent_dir,
    ...
    try:
        ...
        for agent in team:
            children.append(
                start_managed(
                    f"agent-{agent['name']}",
                    team_agent_command(multiagent_dir, agent),
...
def runtime_env_from_context() -> tuple[dict[str, str], Path]:
    env = os.environ.copy()
    root_value = env.get("MULTIAGENT_ROOT", "").strip()
    state_value = env.get("MULTIAGENT_STATE_DIR", "").strip()
    repo_value = env.get("MULTIAGENT_REPO_ROOT", "").strip()
    if root_value and state_value:
        cwd = Path(repo_value or root_value).expanduser().resolve()
        return env, cwd
    repo = discover_repo()
    env.setdefault("MULTIAGENT_ROOT", str(repo.config_dir))
    env.setdefault("MULTIAGENT_REPO_ROOT", str(repo.root))
    env.setdefault("MULTIAGENT_STATE_DIR", str(repo.state_dir))
