...
DEFAULT_IMAGE = "multiagent:local"
PACKAGE = "multiagent"
DEFAULT_REGISTRY_DIR = "~/.multiagent"
CONTAINER_HOME = Path("/home/multiagent")
HOST_CODEX_PROVIDER = "multiagent-host-codex"
...
RUN_LABEL = "multiagent.run=1"
REPO_LABEL = "multiagent.repo"
...
def dockerfile_path() -> Path:
    return Path(str(resources.files(PACKAGE).joinpath("docker_context", "Dockerfile"))).resolve()
...
def docker_common_options(config: StartConfig) -> list[str]:
    ...
    command.extend(["-e", "USER=multiagent"])
    command.extend(["-e", f"PATH={container_path()}"])
    command.extend(["-e", f"MULTIAGENT_HOST_UID={os.getuid()}"])
    command.extend(["-e", f"MULTIAGENT_HOST_GID={os.getgid()}"])
    extra_groups = ":".join(str(group) for group in device_groups(config.devices))
    if extra_groups:
        command.extend(["-e", f"MULTIAGENT_EXTRA_GROUPS={extra_groups}"])
    command.extend(["-e", f"MULTIAGENT_PI_CONFIG_DIR={config.pi_config_dir}"])
    command.extend(["-e", f"MULTIAGENT_REGISTRY_DIR={config.registry_dir}"])
    command.extend(["-e", f"MULTIAGENT_STATE_DIR={config.state_dir}"])
    command.extend(["-e", "MULTIAGENT_CONTAINER=docker"])
    command.extend(["-e", f"MULTIAGENT_CONTAINER_NAME={config.name}"])
...
def start_host_auth_proxy(config: StartConfig, proxy_token: str) -> None:
    ...
    env = os.environ.copy()
    env["MULTIAGENT_HOST_AUTH_TOKEN"] = proxy_token
    command = [
        sys.executable,
        "-m",
        "multiagent.auth_proxy",
        "--socket",
        str(config.auth_socket),
        "--auth-path",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    start.add_argument("--registry-dir", default=os.environ.get("MULTIAGENT_REGISTRY_DIR", DEFAULT_REGISTRY_DIR))
    ...
    stop.add_argument("--registry-dir", default=os.environ.get("MULTIAGENT_REGISTRY_DIR", DEFAULT_REGISTRY_DIR))
    ...
    destroy.add_argument("--registry-dir", default=os.environ.get("MULTIAGENT_REGISTRY_DIR", DEFAULT_REGISTRY_DIR))
