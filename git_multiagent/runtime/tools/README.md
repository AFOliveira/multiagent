...
`agent-pi-interactive`, `git-multiagent-ui`, and the dashboard entry point use
Python 3 stdlib only.

## `git-multiagent-ui`

`git-multiagent-ui` serves the web interface for MULTIAGENT state. It is the
...
GIT_MULTIAGENT_STATE_DIR=~/.multiagent/state/<instance-id> \
...
the state directory explicitly through `GIT_MULTIAGENT_STATE_DIR`:
